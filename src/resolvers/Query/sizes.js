 ...
 * @summary Get a list of all the sizes
 * @param {Object} _ - unused
 * @param {ConnectionArgs} args - an object of all arguments that were sent by the client
 * @param {String[]} args.shopIds - Optional IDs of the shop to get a list of sizes for
 * @param {String[]} args.tagIds - Optional IDs of the tags to get a list of sizes for
 * @param {Object} context - an object containing the per-request state
 * @param {Object} info Info about the GraphQL request
 * @returns {Promise<Object>} A CatalogItemConnection object
 */
export default async function sizes(_, args, context, info) {
    const { shopIds: opaqueShopIds, tagIds: opaqueTagIds, ...connectionArgs } = args;

    const shopIds = opaqueShopIds && opaqueShopIds.map(decodeShopOpaqueId);
    const tagIds = opaqueTagIds && opaqueTagIds.map(decodeTagOpaqueId);

    const { collection, pipeline } = await context.queries.sizes(context, {
        shopIds,
        tagIds
    });
